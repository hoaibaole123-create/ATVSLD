...
/* ------------------------------------------------------------------
 * Phan "hoc theo ngu canh" cho prompt Gemini.
 *
 * LUU Y: doan duoi day duoc CHEP NGUYEN VAN tu api/_learningContext.ts.
 * Vercel build server.ts nhu mot entrypoint doc lap nen khong nap duoc file
 * o thu muc khac (gay FUNCTION_INVOCATION_FAILED). Khi sua logic, nho sua
 * ca hai noi cho khop.
 * ------------------------------------------------------------------ */
/**
...
function buildSheetExamplesContext(sheetExamples: SheetExamplePayload[]): string {
...
function buildLessonsContext(lessons: LessonPayload[]): string {
 ...
 */
function buildLearningContext(params: {
